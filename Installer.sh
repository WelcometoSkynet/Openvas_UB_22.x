...
read -sp "Please enter a password for the admin account:" GVMPASSWORD #(adminadmin)
...
yarn
yarn build  # If you encounter an error Troubleshoot
...
sudo usermod -aG redis gvm  # Check *****
...
sudo -u gvm gvmd --create-user=admin --password=adminadmin
...
cat << EOF > $BUILD_DIR/gsad.service              # Read
[Unit]
...
sudo runuser -u gvm -- greenbone-nvt-sync
...
echo "Login at 127.0.01:9392 with the username admin and your supplied password."
